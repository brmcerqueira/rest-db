...
pub fn group(scope: &mut HandleScope, args: FunctionCallbackArguments, _: ReturnValue) {
    ...
    let (function, key_function): (Local<Function>, Option<Local<Function>>) = if args.length() == 2
    {
        (
            ...
            Some(args.get(0).try_into().unwrap()),
        )
    } else {
        (args.get(0).try_into().unwrap(), None)
    };
    ...
    if key_function.is_some() {
        ...
        for index in 0..array.length() {
            ...
            let key = key_function
                .call(scope, undefined.into(), &[item.into()])
                .unwrap();
    ...
    for item in map {
        let result = function
            .call(scope, item.1.into(), &[item.0.into()])
            .unwrap();
